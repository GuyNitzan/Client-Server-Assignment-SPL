...
import TPCthreadperclient.ConnectionHandler;
...
public class TextGameProtocol implements AsyncServerProtocol<StringMessage> {
	...
	private ConnectionHandler<StringMessage> _handler = null;

	private boolean closing = false;

	public TextGameProtocol(ConnectionHandler<StringMessage> handler) {
		_handler = handler;
	}
	...
	public boolean isEnd(StringMessage msg) {
		...
		if (msg.getMessage().equals("QUIT")) {
			...
			try {
				_handler.sendMessage(new StringMessage("SYSMSG QUIT ACCEPTED"));
			} catch (Exception ex) {
